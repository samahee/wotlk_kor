...
local function loadFullDatabase()
    ...
    -- Merge Project Epoch custom quests/NPCs/objects/items (from pfQuest-epoch)
    -- into the freshly loaded base DB, before corrections & compilation.
    coYield()
    local EpochDB = QuestieLoader:ImportModule("EpochDB")
    if EpochDB and EpochDB.Integrate then
        EpochDB:Integrate()
    end
